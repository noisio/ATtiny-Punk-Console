/*  
  # # # ATtiny85 2-voices bass synth with 4 different wave forms # # #  
               _     _
  ...
  AT85 clock must be set to 16MHz.
*/
...
unsigned int uSteps = 192;                                // microsteps in one octave
...
void setup() {
  ...
  //OSCCAL -= 2;                            // Oscillator calibration
  ...
  OCR0A = 203;                                         // Divide by x+1
}

void loop() {

  unsigned int pot1Read = analogRead(pot1);
  vol = analogRead(pot2) >> 2;
  unsigned int pot3Read = analogRead(pot3);
  unsigned int pot4Read = analogRead(pot4);

  baseNote = (map(pot1Read, 0, 1023, 12, 72) << 4) + uSteps;
  note = pgm_read_word(&tunes1345steps[baseNote]);


  int detune = map(pot3Read, 0, 1023, 0, uSteps);
  if (detune == 0) {
    note2 = note >> 1;
  } else {
